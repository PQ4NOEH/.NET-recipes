window.ALTEA_PROMISES.push(function(window, document, $, undefined) {
    ...
    var api = Object.create({
        setGroupLevels: "/Admin/Groups/SetLevels"
    });

    var dom = Object.create({
        mpdEditGroupLevelsId: "adm-edt-grp-lvl",
        mpdContent: null,
    ...
    function displayLevel(level, container, groupLevel, selected) {
        ...
        check.id = (groupLevel ? "adm-edt-lvl-ulv-" : "adm-edt-lvl-lv-") + level.id;
        ...
        label.htmlFor = (groupLevel ? "adm-edt-lvl-ulv-" : "adm-edt-lvl-lv-") + level.id;
        left.appendChild(label);

        var mid = document.createElement("div");
        mid.className = "adm-edt-lvl-md" + (groupLevel ? " adm-edt-lvl-has-rg" : "");
        //mid.htmlFor = (groupLevel ? "adm-edt-lvl-ulv-" : "adm-edt-lvl-lv-") + level.id;
    ...
    function createEditGroupLevelsPopup(levels) {
        if (window.mpd.created()) {
            window.mpd.destroy();
        }

        dom.mpdContent = window.mpd.create(
            dom.mpdEditGroupLevelsId,
            "800px",
            i18n.t("admin.editgrouplevels"),
            { closebtn: true }
        );

        var allLevels = document.createElement("div");
        allLevels.id = "adm-edt-grp-lvl-alv";
        dom.mpdContent.appendChild(allLevels);

        var allLevelsTitle = document.createElement("div");
        allLevelsTitle.id = "adm-edt-grp-lvl-alv-ttl";
        allLevelsTitle.innerText = i18n.t("Levels");
        allLevels.appendChild(allLevelsTitle);

        var $select = displayAllLevels(levels, allLevels);

        var groupLevels = document.createElement("div");
        groupLevels.id = "adm-edt-grp-lvl-ulv";
        dom.mpdContent.appendChild(groupLevels);

        var groupLevelsTitle = document.createElement("div");
        groupLevelsTitle.id = "adm-edt-grp-lvl-ulv-ttl";
        groupLevelsTitle.innerText = i18n.t("admin.grouplevels");
        groupLevels.appendChild(groupLevelsTitle);

        var groupLevelsData = document.createElement("div");
        groupLevelsData.className = "adm-edt-lvl-lvs";
        groupLevels.appendChild(groupLevelsData);

        for (var i = 0, l = selectedLevels.length; i < l; i++) {
            var level = findLevel(parseInt(selectedLevels[i]), levels);
            if (level !== null) {
                displayLevel(level, groupLevelsData, true, true);
            }
        }

        var save = document.createElement("button");
        save.id = "adm-edt-grp-lvl-cte";
        ...
        $(allLevels).on("change", ".adm-edt-lvl-lvc", function() {
            ...
            if (this.checked) {
                if (l !== null) l.checked = true;
                    ...
                    displayLevel(findLevel(parseInt(lvl), levels), groupLevelsData, true, true);
                ...
                if (primaryLevel === lvl) {
                    var other = groupLevelsData.getElementsByClassName("adm-edt-lvl-lv");
                    if (other.length !== 0) {
        ...
        $(groupLevelsData).on("change", ".adm-edt-lvl-lvc", function() {
            if (this.disabled) {
                return;
            }
        ...
        $(groupLevelsData).on("change", ".adm-edt-lvl-ulv-prm", function() {
            if (this.disabled) {
                return;
            }
    ...
    window.admin.editGroupLevels = function(group, levels, selected, primary) {
        member = group;
        ...
        var $select = createEditGroupLevelsPopup(levels);
